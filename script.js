...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupTouchControls() {
        ...
        // Mobile tap to show back button
        if (state.isMobile) {
            vrContainer.addEventListener('click', handleMobileTap);
        }
    ...
    function handleTouchEnd(e) {
        // Check for swipe gesture to go back to home
    ...
    function handleMobileTap() {
        if (state.isInVRView && !state.isVRMode) { // Only show on tap if not in VR headset mode
            elements.backToHome.classList.toggle('visible-on-tap');
            elements.controlPanel.classList.toggle('visible-on-tap');
            elements.vrNavControls.classList.toggle('visible-on-tap');
            elements.mediaInfo.classList.toggle('visible-on-tap');

            // Hide after a few seconds if no further interaction
            clearTimeout(state.tapTimeout);
            state.tapTimeout = setTimeout(() => {
                elements.backToHome.classList.remove('visible-on-tap');
                elements.controlPanel.classList.remove('visible-on-tap');
                elements.vrNavControls.classList.remove('visible-on-tap');
                elements.mediaInfo.classList.remove('visible-on-tap');
            }, 3000);
        }
    }
    ...
    function disableVRControls() {
        ...
        if (state.isMobile) {
            vrContainer.removeEventListener('click', handleMobileTap);
        }
    ...
    function backToHomePage() {
        ...
        elements.controlPanel.classList.remove('visible-on-tap'); // Remove mobile tap class

        elements.vrNavControls.style.opacity = '0';
        elements.vrNavControls.classList.remove('visible-on-tap'); // Remove mobile tap class

        elements.mediaInfo.classList.remove('visible-on-tap'); // Remove mobile tap class
